...
import { resolveAssigneeId } from '../utils/assignee.js';
 ...
 * @param {string} domain
 * @param {Map<string, Array>} cache - 项目成员缓存（projectId → members[]）
 * @returns {Promise<Array>} 成员列表（拉取失败时返回空数组）
 */
export async function getProjectMembersCached(accessToken, projectId, domain, cache) {
  ...
  try {
    ...
    console.warn(`[ImportHelper] 获取项目 ${projectId} 成员失败，负责人将回退到当前用户:`, e.message);
  }
 ...
 */
function appendSolutionSuggestion(description, solutionSuggestion) {
...
/**
 * 构建单个工作项的 PingCode 创建 payload。
 *
 * @param {object} item - 前端传入的工作项
 * @param {object} ctx - 上下文 { targetProjectId, resolvedTypeId, resolvedPriorityId, members, pingcodeUserId }
 * @returns {object} PingCode work_items POST payload（含 _local_id 用于关联本地记录）
 */
export function buildImportPayload(item, ctx) {
